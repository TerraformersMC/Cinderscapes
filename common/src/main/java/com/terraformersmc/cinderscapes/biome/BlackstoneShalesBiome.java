    ...
    public static Biome create(Registerable<Biome> registerable) {
        return new Biome.Builder()
                ...
                .addEnvironmentAttributes(EnvironmentAttributeMap.builder()
                        .with(EnvironmentAttributes.AMBIENT_PARTICLES_VISUAL, List.of(new AmbientParticle(ParticleTypes.WHITE_ASH, 0.118093334F)))
                        .with(EnvironmentAttributes.AMBIENT_SOUNDS_AUDIO, new AmbientSounds(Optional.of(SoundEvents.AMBIENT_NETHER_WASTES_LOOP), Optional.of(new BiomeMoodSound(SoundEvents.AMBIENT_BASALT_DELTAS_MOOD, 6000, 8, 2.0D)), List.of(new BiomeAdditionsSound(SoundEvents.AMBIENT_BASALT_DELTAS_ADDITIONS, 0.0111D))))
                        .with(EnvironmentAttributes.BACKGROUND_MUSIC_AUDIO, new BackgroundMusic(CinderscapesSoundEvents.BLACKSTONE_SHALES_MUSIC))
                        .with(EnvironmentAttributes.FOG_COLOR_VISUAL, 6235392)
